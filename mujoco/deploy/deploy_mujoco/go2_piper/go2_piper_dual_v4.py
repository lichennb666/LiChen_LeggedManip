...
def main() -> None:
    ...
    policy_stairs = torch.jit.load(root + "/deploy/policy/go2_piper/policy_stairs_v5.pt")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and (time.time() - sim_start) < simulation_duration:
            ...
            if step_counter % control_decimation == 0:
                ...
                _cs = int(step_counter / control_decimation)
                if _cs % 30 == 1 and policy_stairs is not None:
                    heights = []
                ...
                if use_stairs and policy_stairs is not None:
                    hist_obs = torch.cat([
