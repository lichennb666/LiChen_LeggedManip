...
def main() -> None:
    ...
    policy_flat = torch.jit.load(os.path.join(root, "deploy/policy/go2_piper/policy_flat.pt"))
    policy_stairs = torch.jit.load(os.path.join(root, "deploy/policy/go2_piper/policy_stairs_v5.pt"))
    policy = policy_flat  # default
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and (time.time() - sim_start) < simulation_duration:
            ...
            if step_counter % control_decimation == 0:
                ...
                # ── terrain check every 50 steps ───────────────────────────────
            if step_counter % (50 * control_decimation) == 0 and policy_stairs is not None:
                heights = []
                base_pos = data.qpos[0:3].copy()
                R = np.zeros(9)
                mujoco.mju_quat2Mat(R, data.qpos[3:7].copy())
                ...
                for i in range(17):
                    for j in range(11):
                        ...
                        pnt = np.array([base_pos[0]+rx, base_pos[1]+ry, sensor_z]).reshape(3,1)
                        vec = np.array([0.,0.,-1.]).reshape(3,1)
                        gid = np.zeros(1, dtype=np.int32)
                        d = mujoco.mj_ray(model, data, pnt, vec, None, False, -1, gid)
                        heights.append(d if d > 0 else sensor_z)
                terrain_var = np.var(heights)
                use_stairs = terrain_var > 0.01
