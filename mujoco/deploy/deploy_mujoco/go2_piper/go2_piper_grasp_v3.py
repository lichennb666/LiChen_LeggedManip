...
def main() -> None:
    ...
    # G key listener (pynput, GLFW callback unreliable with has_gui)
    ...
    pk.Listener(on_press=_g_press).start()
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and (time.time() - sim_start) < simulation_duration:
            ...
            if step_counter % control_decimation == 0:
                ...
                # ── grasp mode (G key) ───────────────────────────────
                if grab_g_count[0] > 0 and not in_grab:
                    in_grab = True; grab_phase = 0; grab_timer = 0
                    grab_legs = data.qpos[7:19].copy()
                    print(f"\n[GRASP {['REACH','DESCEND','CLOSE','LIFT'][0]}]")
                    grab_g_count[0] = 0
                ...
                if in_grab:
                    ...
                    phases = [(REACH,120),(GRASP,100),(GRASP,60),(HOME,150)]
                    if grab_phase < 4:
                        cur, dur = phases[grab_phase]
                        prev = phases[grab_phase-1][0] if grab_phase>0 else HOME
                        a = min(grab_timer/dur, 1.0)
                        arm = torch.tensor(prev + (cur-prev)*a, dtype=torch.float32)
                        if grab_timer >= dur:
                            if grab_phase == 2: print("[CLOSE GRIPPER]")
                            if grab_phase == 3: print("[GRASP DONE]"); in_grab=False
                            grab_phase += 1; grab_timer = 0
                            if grab_phase >= 4: grab_phase = 0; in_grab = False
                        target_dof_pos = torch.cat([torch.tensor(grab_legs, dtype=torch.float32), arm])

            if step_counter % (50 * control_decimation) == 0:
                rx, ry = data.qpos[0], data.qpos[1]
                d = ((rx-2.0)**2 + (ry+10)**2)**0.5
                s = "GRAB" if in_grab else "WALK"
                print(f"\r  x={rx:.2f} y={ry:.2f}  cube_dist={d:.2f}m  {s}  ", end="")
            viewer.sync()
