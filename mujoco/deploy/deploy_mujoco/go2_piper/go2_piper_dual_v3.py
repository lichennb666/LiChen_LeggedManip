...
def main() -> None:
    ...
    # --- dual-policy: load stairs policy ---
    import pathlib
    _mujoco_root = str(pathlib.Path(__file__).resolve().parent.parent.parent.parent)
    _stairs_path = _mujoco_root + "/deploy/policy/go2_piper/policy_stairs_v5.pt"
    policy_stairs = torch.jit.load(_stairs_path) if pathlib.Path(_stairs_path).exists() else None
    height_scan_obs = torch.zeros(num_env, 187 * num_hist)
    use_stairs = False
    if policy_stairs is not None:
        print("[INFO] Dual-policy: flat(210D) + stairs(771D) loaded")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and (time.time() - sim_start) < simulation_duration:
            ...
            if step_counter % control_decimation == 0:
                ...
                # ── terrain check (every 50 control steps) ──────────────────
                if control_step % 50 == 0 and policy_stairs is not None:
                heights = []
                ...
                for i in range(17):
                    for j in range(11):
                        ...
                        vec=np.array([0.,0.,-1.],dtype=np.float64).reshape(3,1)
                        gid=np.zeros(1,dtype=np.int32)
                        d=mujoco.mj_ray(model,data,pnt,vec,None,False,-1,gid)
                        heights.append(d if d>0 else sz)
                if np.var(heights) > 0.01:
                    if not use_stairs:
                        h_t = torch.tensor(heights).unsqueeze(0)
                        for _h in range(num_hist):
