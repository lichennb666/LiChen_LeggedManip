...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  {
    // FAST-LIO normally makes the initial IMU frame its world frame. That
    // leaves the map tilted when the LiDAR and its built-in IMU are mounted
    // with a non-zero roll or pitch. Rotate measured specific force onto +Z
    // so camera_init is level; the internal LiDAR-to-IMU extrinsic is unchanged.
