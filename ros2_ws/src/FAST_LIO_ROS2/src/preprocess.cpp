...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const double first_point_time = pl_orig.points.front().timestamp;
  const double last_point_time = pl_orig.points.back().timestamp;
  given_offset_time =
      std::isfinite(header_time) && std::isfinite(first_point_time) &&
      std::isfinite(last_point_time) &&
      first_point_time >= header_time - 1.0e-6 &&
      last_point_time >= first_point_time &&
      last_point_time - header_time < 1.0;
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    if (given_offset_time)
    {
      added_pt.curvature = std::max(
          0.0, (pl_orig.points[i].timestamp - header_time) * 1000.0);
